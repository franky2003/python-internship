...
def lol_label():
    content = int(input_element.get())
    sq = content**2
    label2.config(text=f"Square value is {sq}", font=d_font)
...
label1 = tk.Label(text="Enter the number", font=d_font)
label1.grid(row=0, column=0, pady=(10,0), padx=10)

label2 = tk.Label(text="Result here", font=d_font)
